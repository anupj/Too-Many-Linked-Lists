...
impl<T> List<T> {
    ...
    // - each node in the middle of the list is pointed at by
    // its predecessor and successor, while the nodes on the
    // ends are pointed to by the list itself.
    ...
    // `pop_front` should be the same basic logic
    // as `push_front` but backwards
